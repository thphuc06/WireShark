...
\begin{figure}[H]
    ...
    \caption{Timeline của 2 kết nối TCP đến 2 server (statistics -> conversations -> TCP)}
...
Nhìn vào IO Graph phía trên (mục statistics $\rightarrow$ IO Graphs), ta thấy:
\begin{itemize}
...
Trong khi đó, tại giây $6.803$s, packet 1927 có info `Application Data' với kích thước TLS Record là $3632$ bytes -- đây là \textbf{gói tin cuối cùng} chứa dữ liệu ảnh \texttt{pearson.png} được gửi về qua HTTPS (dưới dạng mã hóa TLS). Hai quá trình tải diễn ra song song: ảnh \texttt{cover\_small.jpg} đã hoàn thành ($6.49$s) trong khi \texttt{pearson.png} nhận đủ dữ liệu lúc $6.803$s ở packet 1927.
\newpage
...
Mã trạng thái \textbf{200 OK} là mã phản hồi HTTP phổ biến nhất.

\begin{itemize}
    \item Nó biểu thị rằng \textbf{yêu cầu (request)} của máy khách (client) đã được \textbf{thành công} xử lý.
    \item Đồng thời, máy chủ (server) đã \textbf{cung cấp dữ liệu} được yêu cầu (ví dụ: một trang web, dữ liệu JSON, hình ảnh) trong phản hồi.
\end{itemize}
...
\medskip
\noindent
Nhấp chuột phải vào một packet của từng destination khác nhau, chọn \texttt{Follow} $\rightarrow$ \texttt{TCP Stream} để xem các TCP stream tương ứng.
...
\noindent \textbf{Cờ PSH (Push Flag)}

\begin{itemize}
    \item \textbf{Chức năng:} Trong gói tin này, Server bật cờ PSH để yêu cầu Client chuyển ngay lập tức dữ liệu nhận được lên Lớp Ứng dụng (Trình duyệt) thay vì lưu trữ chờ đầy bộ đệm (TCP Buffer).
    \item \textbf{Mục đích:} Giúp trình duyệt nhận dữ liệu ảnh tức thì để giải mã và hiển thị cho người dùng, loại bỏ độ trễ (latency) không cần thiết.
\end{itemize}
...
\noindent \textbf{Phân tích TCP Window Size:}
\begin{itemize}
    ...
    \item Giá trị 43008 là kích thước vùng đệm nhận của thiết bị gửi gói tin này (trong trường hợp này là Server \texttt{2.56.99.24}).
    \item \textbf{Ý nghĩa:} Thông báo cho thiết bị nhận gói tin này (Client) biết rằng thiết bị gửi (Server) hiện đang có bộ nhớ đệm (buffer) trống là 43008 bytes. Điều này cho phép Client có thể gửi liên tiếp lượng dữ liệu tối đa là 43008 bytes ngược lại cho Server mà không cần dừng lại chờ tín hiệu xác nhận (ACK) cho từng gói.
\end{itemize}

\vspace{0.5em}
\noindent \textbf{Tại sao TCP Window Size quan trọng đối với Lớp Giao vận:}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{part2_mota.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{part2_question1.png}
    \caption{Không có gói tin ARP nào xuất hiện trong cả 2 chu kì DORA của DHCP}
\end{figure}

\begin{itemize}
\item \textbf{Kết luận:} Trong khoảng thời gian của 2 chu kì DORA của DHCP trên, không có gói tin ARP nào được trao đổi.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Tuy nhiên có các gói ARP được ghi nhận xuất hiện ngay sau gói DHCP ACK:}
    \begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Các gói tin DHCP messages}
\end{figure}
...
\textbf{Sự khác biệt rõ ràng nhất nằm ở cách DHCP Server phản hồi lại trong các bước Offer và ACK (dựa trên capture của nhóm):}

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Gói tin} & \textbf{IP đích trong lý thuyết} & \textbf{IP đích trong dữ liệu của nhóm} \\
\hline
...
\textbf{Câu hỏi:} Địa chỉ IP nguồn/đích:\\
...
\begin{itemize}
    \item \textbf{Địa chỉ IP nguồn:} \texttt{192.168.0.103} → Địa chỉ IPv4 của Wi-Fi.
    \item \textbf{Địa chỉ IP đích:} \texttt{8.8.8.8} → Google Public DNS.
\end{itemize}
...
Việc địa chỉ IP Nguồn (Source IP Address) và địa chỉ IP Đích (Destination IP Address) không thay đổi khi gói tin đi qua Internet là một nguyên tắc cốt lõi của thiết kế lớp Mạng (Network Layer), đảm bảo dịch vụ truyền thông end-to-end (từ đầu cuối đến đầu cuối) trên toàn mạng lưới.
    \begin{itemize}
        \item Khi host nguồn tạo datagram, nó gán IP nguồn là địa chỉ IP của chính nó và IP đích là địa chỉ IP của host đích cuối cùng (ở đây là máy chủ DNS 8.8.8.8).
        \item Các router trung gian chỉ thực hiện chức năng chuyển tiếp (forwarding) cục bộ: chúng đọc trường Địa chỉ IP Đích trong header IP, tra bảng chuyển tiếp (forwarding table) để chọn cổng ra thích hợp, rồi gửi tiếp gói tin.
        \item Nếu Địa chỉ IP Đích bị thay đổi giữa chừng, các router kế tiếp sẽ mất thông tin cần thiết để định tuyến gói tin đến host nhận cuối cùng. Do đó, Địa chỉ IP Đích phải giữ nguyên từ host nguồn đến host đích.
        \item Trong trường hợp dùng NAT thì chỉ có Địa chỉ IP Nguồn mới bị thay đổi khi gói tin rời mạng LAN. Tuy nhiên, trong ví dụ này, host nguồn sử dụng địa chỉ IP riêng nên NAT không dùng đến, và Địa chỉ IP Nguồn cũng giữ nguyên.
    \end{itemize}
...
\textbf{Câu hỏi:} Tìm trường Time to Live (TTL) trong phần tiêu đề IP của gói tin DNS Query. Giá trị TTL ban đầu này biểu thị điều gì, và router/cổng (gateway) cục bộ sẽ điều chỉnh giá trị đó như thế nào khi thực hiện chuyển tiếp (forward) gói tin?\\
\textbf{Trả lời:} \\

Đầu tiên, lọc gói DNS Query đầu tiên gửi tới máy chủ DNS bằng filter:
\verb|dns && ip.dst == 8.8.8.8|. Khi mở phần \textbf{Internet Protocol} của gói tin này, trường TTL hiển thị như sau:
\begin{itemize}
    \item \texttt{Time to live: 128}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{part3_question2.png}
    \caption{Gói DNS Query - Trường TTL trong IP header}
    \label{fig:ttl-dns-query}
\end{figure}
\end{itemize}

Giá trị TTL = 128 là \textbf{TTL ban đầu} do host nguồn (máy tính chạy hệ điều hành Windows)
gán khi tạo IP datagram. Trường TTL cho biết \emph{số bước nhảy tối đa} (max hops) mà gói tin
được phép đi qua trước khi bị loại bỏ, giúp ngăn chặn các \emph{vòng lặp định tuyến} khiến
gói tin tồn tại vô hạn trên mạng.

\medskip

\noindent
	\textbf{Cách default gateway cục bộ xử lý TTL:}
Default gateway (router Wi-Fi trong mạng LAN) là \textbf{bước nhảy (hop) đầu tiên} trên
đường đi của gói DNS Query ra Internet. Tại gateway này, TTL được xử lý theo chuẩn như sau:
\begin{enumerate}
    \item Router nhận gói IP với TTL ban đầu là 128.
    \item Router \textbf{giảm TTL đi 1} rồi mới chuyển tiếp (forward) gói tin:
    \begin{itemize}
        \item Khi gói rời khỏi mạng LAN, TTL còn \textbf{127}.
    \end{itemize}
    \item Mỗi router trung gian tiếp theo trên đường đi tiếp tục lặp lại quy trình này,
    mỗi hop lại giảm TTL xuống 1.
    \item Nếu tại một router nào đó, sau khi giảm mà TTL trở thành \textbf{0}, router sẽ:
    \begin{itemize}
        \item Loại bỏ (drop) gói tin.
        \item Gửi ngược về host nguồn một bản tin \texttt{ICMP Time Exceeded} để báo gói tin
        đã hết “tuổi thọ” trên mạng.
    \end{itemize}
\end{enumerate}

Trong capture trên Wireshark, chỉ quan sát được TTL = 128 tại thời điểm gói vừa rời host nguồn.
Tuy nhiên, về mặt hoạt động chuẩn, ngay khi gói đi qua default gateway cục bộ, TTL của gói DNS
Query đã được giảm xuống còn 127.


\newpage
\subsubsection{Câu 3: Địa chỉ MAC của Router/Gateway}
\textbf{Câu hỏi:} Xác định địa chỉ MAC của router/cổng mặc định (gateway) cục bộ của bạn (bằng cách sử dụng lệnh arp -a trong cửa sổ dòng lệnh, hoặc bằng cách tìm địa chỉ MAC gắn với địa chỉ IP gateway trong file bắt gói tin (capture)).\\

\textbf{Trả lời:}

\begin{itemize}
    \item Địa chỉ IP của router (Default Gateway) thu được từ lệnh \texttt{ipconfig}
  trong Command Prompt là \texttt{192.168.0.1}: 
    ...
    \item Địa chỉ MAC của router/gateway cục bộ, xác định từ kết quả lệnh \texttt{arp -a} tương ứng với IP Gateway \texttt{192.168.0.1}, là \texttt{5c-a6-e6-e1-90-01}:
...
\textbf{Câu hỏi:} Các địa chỉ nguồn và đích trong tiêu đề tầng Liên kết (Link Layer) (Ethernet hoặc 802.11) là gì? Về bản chất, những địa chỉ này khác gì so với các địa chỉ IP mà bạn đã tìm được ở tầng Mạng?
...
\begin{itemize}
    \item \textbf{Địa chỉ MAC nguồn:} \texttt{08-6a-c5-be-fe-60} → card Wi-Fi của laptop.
    \item \textbf{Địa chỉ MAC đích:} \texttt{5c-a6-e6-e1-90-01} → router/gateway Wi-Fi.
\end{itemize}
...
\textbf{Câu hỏi:} Quan sát tiêu đề tầng Liên kết (Ethernet II hoặc tương tự). Giá trị của trường Type là bao nhiêu, và trường này cho thiết bị nhận (router/gateway của bạn) biết điều gì về loại giao thức tầng trên (protocol) nằm ngay sau tiêu đề Ethernet?\\
...
\begin{itemize}
    \item \textbf{Cơ chế tối ưu hiệu năng của trình duyệt (Concurrency):} Giúp nhận thấy trình duyệt không tải dữ liệu một cách tuần tự mà thực hiện tải song song các tài nguyên (như hình ảnh) để giảm độ trễ.
    
    \item \textbf{Quy trình thiết lập kết nối tin cậy (3-Way Handshake):} Trước khi bất kỳ dữ liệu HTTP nào (như GET request) được truyền đi, thấy rõ quy trình "bắt tay ba bước" để đồng bộ hóa. Điều này đảm bảo kết nối được thiết lập chắc chắn trước khi trao đổi dữ liệu tầng ứng dụng.
    
    \item \textbf{Vai trò của Window Size trong tầng giao vận (Flow Control):} Thông qua giá trị Calculated window size, hiểu được cách TCP thực hiện kiểm soát luồng. Giá trị này thông báo cho bên gửi biết dung lượng bộ nhớ đệm còn trống của bên nhận. Đây là cơ chế thiết yếu để ngăn chặn việc gửi dữ liệu quá nhanh gây tràn bộ đệm (buffer overflow), từ đó giảm thiểu việc mất gói tin và tắc nghẽn mạng.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Hiểu rõ sự khác biệt và vai trò của IP và MAC:} Qua bài tập giúp hiểu rõ rằng IP là địa chỉ logic dùng để định tuyến end-to-end, nên không thay đổi khi đi qua Internet; còn MAC là địa chỉ vật lý trong LAN, và sẽ thay đổi ở mỗi hop. Điều này giúp em nắm chắc mối quan hệ giữa Layer 2 và Layer 3 trong mạng.
    
    \item \textbf{Nắm vững cơ chế TTL và lý do phải giảm tại mỗi router:} TTL giúp ngăn gói tin chạy vòng lặp vô hạn. Mỗi router giảm TTL xuống 1; nếu TTL = 0, gói bị drop và gửi ICMP Time Exceeded. Bài tập giúp hiểu chính xác cách router xử lý gói IP và vì sao TTL là thông số quan trọng khi phân tích mạng.
    
    \item \textbf{Biết cách phân tích gói tin và xác định router/gateway bằng Wireshark:} Học được cách dùng Wireshark và \texttt{arp -a} để nhận diện MAC của gateway, đọc Ethernet header, EtherType và các trường IP/UDP/DNS. Nhờ đó mà tự tin hơn trong việc phân tích các gói tin và hiểu rõ quá trình encapsulation từ Layer 2 $\rightarrow$ Layer 3 $\rightarrow$ Layer 4 $\rightarrow$ Layer 7.
\end{itemize}
