...
Có tổng cộng 3 yêu cầu HTTP GET được trình duyệt gửi đi, không tính \texttt{favicon.ico}:
\begin{itemize}
...
\noindent\textit{(Note: You should ignore any HTTP GET and response for \texttt{favicon.ico}. If you see a reference to this file, it is your browser automatically asking the server if it (the server) has a small icon file that should be displayed next to the displayed URL in your browser. We'll ignore references to this pesky file in this lab.)} (Ross, 2005-2016)

Các yêu cầu này được gửi đến 2 địa chỉ Internet khác nhau:
\begin{itemize}
    \item \texttt{128.119.245.12} - cho file HTML và \texttt{pearson.png}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Mở ứng dụng Windows Command Prompt (CMD) bằng quyền Quản trị viên (Run as administrator).
        \item Nhập lệnh \texttt{ipconfig /release} để yêu cầu Host giải phóng địa chỉ IP hiện tại đang sử dụng, chuẩn bị cho quá trình xin cấp phát mới.
    \end{itemize}
    
    \item \textbf{Bước 2: Bắt đầu ghi lại dữ liệu}
    \begin{itemize}
        \item Khởi động ứng dụng phân tích gói tin Wireshark.
        \item Bắt đầu ghi lại dữ liệu gói tin (packet capture) trên giao diện mạng đang hoạt động của Host.
    \end{itemize}
    
    \item \textbf{Bước 3: Chu kỳ cấp phát IP lần 1}
    \begin{itemize}
        \item Quay lại Command Prompt, nhập lệnh \texttt{ipconfig /renew}.
        \item Lệnh này hướng dẫn Host bắt đầu quy trình DHCP để yêu cầu cấp phát một cấu hình mạng mới (bao gồm một địa chỉ IP mới).
    \end{itemize}
    
    \item \textbf{Bước 4: Chu kỳ cấp phát IP lần 2}
    \begin{itemize}
        \item Sau khi lệnh trước kết thúc, nhập lại lệnh \texttt{ipconfig /renew}.
        \item Lệnh này được thực hiện để quan sát sự khác biệt trong quá trình trao đổi gói tin DHCP khi máy khách đã có địa chỉ IP.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Lệnh này hướng dẫn Host giải phóng địa chỉ IP đã được cấp phát trở lại cho DHCP Server.
    \end{itemize}
    
    \item \textbf{Bước 6: Chu kỳ cấp phát IP lần cuối}
    \begin{itemize}
        \item Cuối cùng, nhập lệnh \texttt{ipconfig /renew} lần nữa.
        \item Lệnh này tạo ra một chu kỳ DHCP đầy đủ khác.
    \end{itemize}
    
    \item \textbf{Bước 7: Kết thúc ghi dữ liệu}
    \begin{itemize}
        \item Quay lại Wireshark và dừng quá trình ghi lại gói tin.
        \item Lưu lại bản ghi gói tin để phục vụ cho việc phân tích.
        \item Áp dụng bộ lọc \texttt{dhcp || arp} để phân tích các gói tin DHCP và ARP.
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Kết quả capture gói tin DHCP và ARP}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{part2_question1_1.png}
    \caption{Chu kỳ DHCP DORA thứ nhất và thứ hai}
\end{figure}

\begin{itemize}
    \item \textbf{Chu kỳ thứ nhất:}
    \begin{itemize}
        \item Bắt đầu (Discover): Gói tin 193 tại thời điểm 17.243646 giây.
        \item Kết thúc (ACK): Gói tin 197 tại thời điểm 17.292309 giây.
        \item Thời gian hoàn tất: 17.292309 - 17.243646 = 0.048663 giây.
    \end{itemize}
    
    \item \textbf{Chu kỳ thứ hai:}
    \begin{itemize}
        \item Bắt đầu (Discover): Gói tin 1520 tại thời điểm 61.961858 giây.
        \item Kết thúc (ACK): Gói tin 1524 tại thời điểm 62.005171 giây.
        \item Thời gian hoàn tất: 62.005171 - 61.961858 = 0.043313 giây.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Phân giải địa chỉ MAC của Gateway:} Sau khi nhận địa chỉ IP và biết địa chỉ Gateway qua DHCP ACK, máy phải sử dụng ARP để tìm địa chỉ MAC tương ứng của Gateway. Đây là bước bắt buộc để máy có thể đóng gói dữ liệu và gửi lưu lượng truy cập ra khỏi mạng cục bộ. Các gói ARP liên quan: ARP Request (Who has 192.168.1.1? Tell 192.168.1.101) và ARP Reply (ví dụ: Gói 200, 201,...).
        
        \item \textbf{Kiểm tra xung đột IP (ARP Probe):} Máy gửi một ARP Probe cho chính địa chỉ IP vừa được cấp phát. Nếu nhận được bất kỳ phản hồi nào, nó sẽ biết có sự xung đột địa chỉ IP (IP Conflict) và sẽ từ bỏ IP đó để bắt đầu lại quá trình DHCP. Các gói ARP liên quan: ARP Request (ví dụ: Who has 192.168.1.101?).
        
        \item \textbf{Định danh địa chỉ mới (ARP Announcement):} Máy có thể chủ động thông báo cho các thiết bị khác trong mạng rằng địa chỉ MAC đã được gán cho 1 địa chỉ IP. Điều này giúp các máy khác cập nhật nhanh chóng bảng ARP của chúng và tránh sự chậm trễ khi giao tiếp lần đầu. Gói tin liên quan: ARP Announcement (ARP Announcement for 192.168.1.101).
    \end{itemize}
\end{itemize}
...
\textbf{Phân tích các địa chỉ IP được Host Client sử dụng trong các bước Discover và Request:}
...
\hline
IP Nguồn: 0.0.0.0 & Báo hiệu máy chưa có IP. Lệnh Discover và Request được gửi khi ngăn xếp IP (IP Stack) của Client chưa được khởi tạo đầy đủ. 0.0.0.0 là địa chỉ "phi cấu hình" (unspecified address) để Host có thể tạo và gửi gói tin ở Lớp Mạng mà không cần IP hợp lệ. \\
\hline
...
\begin{itemize}
    \item \textbf{Hiệu quả (Efficiency):} Server đã quyết định gửi Offer và ACK trực tiếp đến địa chỉ IP mà nó đề xuất/vừa gán thay vì gửi Broadcast để giảm lưu lượng Broadcast trên mạng.
    
    \item \textbf{Khẳng định MAC:} Server có thể làm điều này vì nó đã biết địa chỉ MAC của Client (được gửi trong gói Discover). Mặc dù IP chưa được Client xác nhận chính thức, Server vẫn có thể đóng gói dữ liệu và gửi đến địa chỉ MAC của Client.
    
    \item \textbf{Tính tạm thời của IP:} Việc sử dụng địa chỉ 192.168.1.101 ngay từ gói Offer nhấn mạnh rằng địa chỉ IP đó, mặc dù chưa được xác nhận, đã có giá trị logic và được sử dụng để định danh Host trong phần còn lại của quá trình.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{part2_question3.png}
    \caption{ARP Probe và xử lý xung đột IP theo RFC 2131}
\end{figure}

\begin{itemize}
    \item \textbf{Cơ chế:} Host gửi một hoặc nhiều gói tin ARP Probe (ARP thăm dò) cho chính địa chỉ mới được cấp. Đây là các gói ARP không có địa chỉ IP nguồn để thăm dò.
    
    \item \textbf{Tình huống xung đột:} Khi Host nhận được gói ARP Reply từ một thiết bị khác trên mạng, điều đó chứng tỏ địa chỉ IP đó đã được sử dụng.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item DHCP Request là một phần của quá trình DORA (thường xảy ra sau Discover/Offer). Trong bước Request, Host Client vẫn đang sử dụng Source IP là 0.0.0.0 (vì chưa có IP chính thức).
        
        \item Trong tình huống mà thông tin Lớp Mạng không đầy đủ, việc sử dụng cổng cố định Port 68 là quan trọng. Nó đảm bảo rằng khi Server phản hồi lại bằng gói DHCP ACK, nó biết chính xác Port nào (Port 68) mà ứng dụng DHCP Client đang lắng nghe trên Host.
    \end{itemize}
\end{itemize}
...
\textbf{Câu hỏi:} Kiểm tra trường Checksum UDP trong gói DHCP Discover. Nếu Checksum không chính xác, Lớp Giao vận (Transport Layer) sẽ thực hiện hành động gì?
...
\begin{itemize}
    ...
    \item \textbf{Hậu quả khi Capture:} Wireshark, chạy ở Lớp Liên kết dữ liệu (Data Link Layer) hoặc ngay trên Lớp Mạng (Network Layer), bắt giữ gói tin trước khi card mạng NIC kịp hoàn thành việc tính toán Checksum. Do đó, giá trị Checksum mà Wireshark ghi lại có thể là giá trị tạm thời, không chính xác, hoặc chỉ là giá trị unverified (0xd490), khiến Wireshark không thể xác minh tính toàn vẹn của gói tin.
\end{itemize}

\textbf{b. Hành động của Transport Layer khi Checksum không chính xác:}

Nếu Checksum của gói tin DHCP Discover được tính toán là không chính xác (incorrect), Lớp Giao vận (UDP handler) của bên nhận sẽ ngay lập tức hủy gói tin, UDP handler sẽ loại bỏ (drop) gói tin đó tại lớp Transport và không chuyển dữ liệu lên DHCP Server.

\textbf{Lý do:}
\begin{itemize}
    \item \textbf{Tính toàn vẹn dữ liệu:} Checksum được sử dụng để kiểm tra tính toàn vẹn (integrity) của dữ liệu. Nếu Checksum bị sai, điều đó có nghĩa là dữ liệu bên trong gói tin đã bị thay đổi hoặc hỏng trong quá trình truyền tải.
    
    \item \textbf{Không có cơ chế sửa lỗi:} UDP là giao thức phi kết nối (connectionless) và không đáng tin cậy (unreliable). Không giống như TCP, UDP không có cơ chế sửa lỗi, không có khả năng yêu cầu gửi lại gói tin, hay thông báo lỗi cho bên gửi. Do đó, hành động duy nhất và bắt buộc là loại bỏ gói tin bị lỗi để tránh chuyển dữ liệu hỏng.
\end{itemize}

\textbf{Lý do quá trình DHCP bị đình trệ (stall):}

\begin{itemize}
    \item \textbf{Gói Discover không đến được Server:} Gói DHCP Discover bị loại bỏ ở Lớp Giao vận (Transport Layer) của Server, nghĩa là DHCP Server không bao giờ nhận được yêu cầu cấp IP từ Client.
